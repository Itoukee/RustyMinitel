...
fn main() {
    ...
    siv.add_layer(Dialog::text("Select :")
                        .title("RustyMinitel")
...
fn information(s: &mut Cursive) {
    ...
    s.pop_layer();
    s.add_layer(Dialog::text("Informations User")
...
fn network(s: &mut Cursive) {
    s.pop_layer();
    s.add_layer(Dialog::text("Network User")
                .title("RustyMinitel / Network")
                .button("Return Menu", Cursive::quit));

}

fn process(s: &mut Cursive) {
    s.pop_layer();
    s.add_layer(Dialog::text("Process")
